...
public class EventBriteAuthUser extends BasicOAuth2AuthUser implements
	...
	public EventBriteAuthUser(final JsonNode node,
		...
		if (node.has(Constants.EMAILS)) {
			...
			Logger.debug("Has email node : " + settingsNode.has(Constants.EMAIL));
			for(JsonNode jsonNode : settingsNode)
			{
				Logger.debug("Verified " + jsonNode.get(Constants.PRIMARY).asBoolean() );
